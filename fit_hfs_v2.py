...
######## import the spectrum ########
print("Loading the observed spectra...")
# unit for spectrum
unit = "K"
...
def log_likelihood(param):
    dv_FWHM, v0, N, T, s = populate_params(param, transition, fit_type)
...
with Pool(processes=14) as pool:
    hammer = EmceeHammer(
        ...
        nwalker=nwalker,
        nstep=nstep,
        initial_state_blob_mag=1e-2
    )
    hammer.run(
        pool=pool,
        savefilename=fitfilepath
        + "{:s}_{:s}_{:s}_v2.h5".format(source, "_".join(transition), fit_type),
    )
...
# savefile = fitfilepath + "{:s}_{:s}_{:s}_v2_test.pkl".format(
#     source, "_".join(transition), fit_type
# )
# with open(savefile, "wb") as f:
#     pickle.dump(hammer, f, protocol=pickle.HIGHEST_PROTOCOL)
